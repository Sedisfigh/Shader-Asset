...
extern MY_HIGHP_OR_MEDIUMP vec2 absoluteglitch_balavirus;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float POLY_THROWAWAY = (.5 + .5* cos( (absoluteglitch_balavirus.x) * 2.612 + ( 0.2 + -.5 ) *3.14));
    float POLY_THROWAWAY_2 = POLY_THROWAWAY + absoluteglitch_balavirus.y * 0.06;
    ...
    texCoordsR.x += (0.124 * rand(vec2(iTime, texCoordsR.y))) - 0.001 + (POLY_THROWAWAY * 0.0000001);
