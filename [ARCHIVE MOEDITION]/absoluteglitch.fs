...
extern MY_HIGHP_OR_MEDIUMP vec2 absoluteglitch;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float POLY_THROWAWAY = (.5 + .5* cos( (absoluteglitch.x) * 2.612 + ( 0.2 + -.5 ) *3.14));
    float POLY_THROWAWAY_2 = POLY_THROWAWAY + absoluteglitch.y * 0.06;
