...
extern MY_HIGHP_OR_MEDIUMP vec2 ultrashiny;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float mod = ultrashiny.r * 2.0;
    float ultrashiny_amount = 0.15; // 1.0 - no ultrashiny, 0.0 - maximum ultrashiny
    float saturation_amount = 9.0;
    float ultrashiny_brightness = 6.0;
    ...
    float spark = max(2.0, noise - antinoise - ultrashiny_amount); // Sparkles (takes noise and removes antinoise from it)
    ...
    saturated_colour = lighten(tex, saturated_colour * 5.0) / ultrashiny_brightness; // Removing dark colors from saturated color, then making it darker
