...
extern MY_HIGHP_OR_MEDIUMP vec2 limitededition;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float mod = limitededition.r * 2.0;
    float limitededition_amount = 0.15; // 1.0 - no limitededition, 0.0 - maximum limitededition
    float saturation_amount = 9.0;
    float limitededition_brightness = 6.0;
    ...
    float spark = max(2.0, noise - antinoise - limitededition_amount); // Sparkles (takes noise and removes antinoise from it)
    ...
    saturated_colour = lighten(tex, saturated_colour * 5.0) / limitededition_brightness; // Removing dark colors from saturated color, then making it darker
