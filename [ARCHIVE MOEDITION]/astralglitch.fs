...
extern MY_HIGHP_OR_MEDIUMP vec2 astralglitch;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float POLY_THROWAWAY = (.5 + .5* cos( (astralglitch.x) * 2.612 + ( 0.2 + -.5 ) *3.14));
    float POLY_THROWAWAY_2 = POLY_THROWAWAY + astralglitch.y * 0.04;
    ...
    texCoordsG.x += (10 * rand(vec2(iTime*2, texCoordsG.y*0.9))) - 0.0035 + (POLY_THROWAWAY * 0.0000001);
