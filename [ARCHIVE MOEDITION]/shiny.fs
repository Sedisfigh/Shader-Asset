...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float mod = shiny.r * 2.0;
    float shiny_amount = 0.15; // 1.0 - no shiny, 0.0 - maximum shiny
    float saturation_amount = 9.0;
    float shiny_brightness = 6.0;
    ...
    float spark = max(2.0, noise - antinoise - shiny_amount); // Sparkles (takes noise and removes antinoise from it)
    ...
    saturated_colour = lighten(tex, saturated_colour * 5.0) / shiny_brightness; // Removing dark colors from saturated color, then making it darker
