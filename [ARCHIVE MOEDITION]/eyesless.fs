...
// Card rotation
extern PRECISION vec2 eyesless;
...
// extern PRECISION number eyesless_radius;
...
{
    // GAUSSIAN eyesless SETTINGS {{{
    float direction = 16.0; // eyesless DIRECTIONS (Default 16.0 - More is better but slower)
    float quality = 6.0;    // eyesless QUALITY (Default 4.0 - More is better but slower)
    float size = 4.0;       // eyesless SIZE (radius)
    // GAUSSIAN eyesless SETTINGS }}}
    ...
    vec4 eyeslessred_pixel = Texel(texture, texture_coords);
    
    // eyesless calculations
    ...
    for (float d = 0.0; d < two_pi; d += d_step) {
		for (float i = i_step; i < 1.001; i+=i_step) {
			eyeslessred_pixel += Texel( texture, texture_coords + vec2(cos(d), sin(d)) * radius * i);
        }
    }
    
    // Final processing 
    eyeslessred_pixel /= quality * direction + 1.;
    vec4 final_pixel =  vec4(eyeslessred_pixel.rgb, original_pixel.a);

    vec4 hsl = HSL(final_pixel);

    // Adjust lightness for a glossy effect

    float t = eyesless.y*4.221 + mod(time,1);
    ...
    float res = (.5 + .5* cos( (eyesless.x) * 2.612 + ( field + -.5 ) *3.14));
    // Mostly use original lightness, with slight change from moving the card & with time
	hsl.z = 0.7*hsl.z + sin(hsl.z/2.5 - res/4 + sin(eyesless.y)/8 + 0.5)/3.;
