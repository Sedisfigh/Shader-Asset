...
extern MY_HIGHP_OR_MEDIUMP vec2 sparkle;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float mod = sparkle.r * 2.0;
    float sparkle_amount = 0.15; // 1.0 - no sparkle, 0.0 - maximum sparkle
    float saturation_amount = 5.0;
    float sparkle_brightness = 3.0;
    ...
    float spark = max(0.0, noise - antinoise - sparkle_amount); // Sparkles (takes noise and removes antinoise from it)
    ...
    saturated_colour = lighten(tex, saturated_colour * 5.0) / sparkle_brightness; // Removing dark colors from saturated color, then making it darker
