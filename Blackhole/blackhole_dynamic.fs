...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float x = 20*(uv.x+mouse_offset.x-0.5);
    float y = -(uv.y+mouse_offset.y-0.5)*17*1.338;
    ...
    if (sqrt(x*x + y*y) >= 2){
        colour.r = colour.r*(1 - 3/(sqrt(x*x + y*y) + 1));
        colour.g = colour.g*(1 - 3/(sqrt(x*x + y*y) + 1));
        colour.b = colour.b*(1 - 3/(sqrt(x*x + y*y) + 1));
    }
    ...
    float black_holey = 6/(0.2 + sqrt(scaled_uvx*scaled_uvx + scaled_uvy*scaled_uvy));
    float black_holex = 6/(0.2 + sqrt(scaled_uvx*scaled_uvx + scaled_uvy*scaled_uvy));
